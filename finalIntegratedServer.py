...
def post_admin():
    ...
    try: 
        ...
        uid = c.execute('SELECT id FROM Admins WHERE username=?;',(username,)).fetchone()
        if uid is None:
            flash("Username is not associated with an admin account")
            return redirect(url_for("get_admin"))
        savedhash = c.execute('SELECT passwordhash FROM Admins WHERE id=?;',(uid[0],)).fetchone()
